...
object SnakeCreator {
  ...
  def initialY(implicit  screenSize: ScreenSize): Int = 3 * screenSize.ymax / 4
  def initialBody(implicit screenSize: ScreenSize): List[Position] = (0 to 3).map((dy: Int) => Position(initialX, initialY + 10 * dy)).toList.reverse
